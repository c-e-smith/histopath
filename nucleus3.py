...
ROOT_DIR = os.path.abspath("/home/user/ec2-user/histopath")
...
class NucleusDataset(utils.Dataset):
    ...
    def fix_id(self,training_path):
        """
        takes path, strips it to ID
        """
        i = str(training_path)[-9:-8]
        if str(training_path)[29:30].lower() == "m":
            M = 1
    ...
    def load_nucleus(self, dataset_dir, subset, name="nucleus"):
        ...
        self.add_class(name, 1, subset)

        # Which subset?
        # "val": use hard-coded list below:
        ...
        image_ids = [fix_id(path) for path in pathlist]
        ...
        for image_id in image_ids:
            self.add_image(
                name,
                image_id=image_id,
                path=pathlist[image_ids.index(image_id)])
